...
var UpdateUserItem = React.createClass({ 
  ...
  handleUpdate: function() {
    ...
    fd.append("item[description]", document.getElementById("description").value)
   	var self = this;
   	var url = "http://rentmymusic.herokuapp.com/user_profile/1/items/"+this.props.itemtoupdate.id+".json"
   	axios({
      method: 'put',
      url: url,
      processData: false,
  ...
  imageUpdation: function(){
    ...
    for (var thumbId in item.thumbnail){
      arr.push(
       		   <div className ="wrap">
       		   <img src={"http://rentmymusic.herokuapp.com"+item.thumbnail[thumbId]} key= {"img"+thumbId} />
